...
import { Building2, MapPin, CalendarDays, Search, Filter, Clock, Users, BookOpen, Trophy } from "lucide-react";
...
export default function LyceesPage() {
  ...
  const ProgrammeComplet = ({ lycee }: ProgrammeCompletProps) => {
    ...
    if (!lycee) {
      return <div className="text-center py-8 text-gray-500">Aucun lycée sélectionné</div>;
    }
    ...
    const programmeDetaille = programmes.length > 0 ? programmes : [
      {
        id: '1',
    ...
    const getTypeIcon = (type: string) => {
      switch (type) {
        case 'ACCUEIL': return <Users className="h-4 w-4" />;
        ...
        case 'RENCONTRE': return <Users className="h-4 w-4" />;
        case 'REPAS': return <Clock className="h-4 w-4" />;
    ...
    const getTypeColor = (type: string) => {
      switch (type) {
        ...
        case 'RENCONTRE': return 'bg-pink-100 text-pink-800 border-pink-200';
        case 'REPAS': return 'bg-yellow-100 text-yellow-800 border-yellow-200';
        case 'DEMONSTRATION': return 'bg-indigo-100 text-indigo-800 border-indigo-200';
        case 'FORUM': return 'bg-red-100 text-red-800 border-red-200';
        ...
        <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg p-6 border border-blue-200">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
            ...
            <div className="flex items-center gap-2">
              ...
              <span><strong>Type:</strong> {programmeDetaille.length > 0 && programmes.length > 0 ? 'Données réelles' : 'Programme type'}</span>
            </div>
            <div className="flex items-center gap-2">
              <MapPin className="h-4 w-4 text-blue-600" />
              <span><strong>Lieux:</strong> {[...new Set(programmeDetaille.map(p => p.lieu))].length} espaces</span>
            </div>
          </div>
        </div>
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      <div className="flex items-center gap-2">
                        <span className="font-semibold text-blue-600">{activite.heure}</span>
        ...
        <div className="bg-gray-50 rounded-lg p-6 border border-gray-200">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
            ...
            <div>
              ...
              <p className="text-gray-600">
                Aucune réservation nécessaire pour la plupart des activités. 
                Les ateliers en petits groupes peuvent nécessiter une inscription sur place.
              </p>
            </div>
            <div>
              <h4 className="font-medium mb-2">Contact</h4>
              <p className="text-gray-600">
                Pour toute question: {lycee.nom.toLowerCase().replace(/\s+/g, '.')}@education.ma<br />
            ...
            <div>
              ...
              <p className="text-gray-600">
                ...
                Merci de nous prévenir pour une meilleure organisation.
              </p>
            </div>
          </div>
        </div>
    ...
    <Layout>
      ...
      <div className="bg-gradient-to-r from-blue-600 to-blue-800 text-white py-12 md:py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h1 className="text-3xl md:text-4xl lg:text-5xl font-bold mb-4">Lycées Participants</h1>
          <p className="text-lg md:text-xl text-blue-100 max-w-3xl mx-auto px-4">
            Découvrez les établissements d'accueil qui ouvrent leurs portes pour votre orientation
          </p>
        </div>
      </div>
        ...
        <div className="bg-white rounded-lg shadow-sm p-4 md:p-6 mb-6 md:mb-8">
          <div className="flex flex-col md:flex-row gap-4 items-center">
            ...
            <div className="text-sm text-gray-600 w-full md:w-auto text-center md:text-left">
              {filteredLycees.length} lycée{filteredLycees.length > 1 ? 's' : ''} trouvé{filteredLycees.length > 1 ? 's' : ''}
            </div>
          </div>
        </div>
        ...
        {filteredLycees.length === 0 ? (
          <Card>
            <CardContent className="text-center py-12 animate-fadeIn">
              ...
              <h3 className="text-lg font-medium text-gray-900 mb-2">
                {lycees.length === 0 ? "Aucun lycée disponible" : "Aucun lycée trouvé"}
              </h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
            {filteredLycees.map((lycee, index) => (
              <Card key={lycee.id} className="hover:shadow-lg transition-all-300 card-animate hover-lift">
                <CardHeader>
                  <div className="flex items-start justify-between">
                    <div className="flex-1 pr-2">
                      <CardTitle className="text-base md:text-lg">{lycee.nom}</CardTitle>
                    ...
                    <Badge variant={lycee.type === "PUBLIC" ? "default" : "secondary"} className="text-xs flex-shrink-0 badge-animate">
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <div className="space-y-1">
                        {lycee.evenements.slice(0, 2).map((evenement) => (
                          <div key={evenement.id} className="text-xs text-gray-600 list-item-animate">
                            {formatDate(evenement.date)} - {formatTime(evenement.heureDebut)} à {formatTime(evenement.heureFin)}
                          </div>
                        ))}
                        {lycee.evenements.length > 2 && (
                          <div className="text-xs text-blue-600">
                            +{lycee.evenements.length - 2} autre{lycee.evenements.length > 3 ? 's' : ''} date{lycee.evenements.length > 3 ? 's' : ''}
                          </div>
                        )}
                      </div>
                    </div>
                    ...
                    <Dialog>
                      <DialogTrigger asChild>
                        <Button className="w-full button-animate hover-lift" size="sm" onClick={() => setSelectedLycee(lycee)}>
                        ...
                        <div className="space-y-6">
                          <div className="flex items-center gap-2 animate-fadeIn animate-delay-200">
                            <Badge variant={lycee.type === "PUBLIC" ? "default" : "secondary"}>
                              {lycee.type === "PUBLIC" ? "Établissement Public" : "Établissement Privé"}
                            </Badge>
                          </div>
                          ...
                          <div className="animate-fadeIn animate-delay-500">
                            <h4 className="font-medium mb-3 flex items-center gap-2">
                              ...
                              Calendrier des événements ({lycee.evenements.length})
                            </h4>
                            <div className="space-y-2">
                              {lycee.evenements.map((evenement) => (
                                <div key={evenement.id} className="bg-gray-50 rounded-lg p-3 list-item-animate">
                    ...
                    <Dialog open={showProgrammeComplet} onOpenChange={setShowProgrammeComplet}>
                      <DialogContent className="max-w-4xl max-h-[85vh] overflow-y-auto">
                        <DialogHeader>
                          ...
                          <DialogDescription className="animate-fadeIn animate-delay-100">
                            Détail des activités et événements prévus pendant les portes ouvertes
                          </DialogDescription>
                        </DialogHeader>
