...
export async function PUT(request: NextRequest, context: RouteContext) {
  try {
    ...
    console.log('📝 Données reçues pour mise à jour:', { id, nom, adresse, type, hasDescription: !!description, hasLogo: !!logo });
    ...
    console.log(`✅ Lycée mis à jour: ${nom} (ID: ${id}) - ${new Date().toISOString()}`);
...
export async function DELETE(request: NextRequest, context: RouteContext) {
  try {
    ...
    console.log(`✅ Lycée supprimé: ${existingLycee.nom} (ID: ${id}) - ${new Date().toISOString()}`);
