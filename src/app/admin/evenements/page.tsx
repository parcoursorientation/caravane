  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        toast({
          title: "Succès",
          description: editingEvenement ? "Événement mis à jour avec succès" : "Événement créé avec succès",
          variant: "default",
        });
        fetchEvenements();
        resetForm();
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Gérez les sessions de l'événement avec leurs lycées et exposants associés
            </CardDescription>
          </CardHeader>
          <CardContent>
            {filteredEvenements.length === 0 ? (
              <div className="text-center py-12">
                ...
                <p className="text-gray-500 mb-4">
                  {searchTerm ? "Aucun événement ne correspond à votre recherche." : "Commencez par ajouter un événement."}
                </p>
                {!searchTerm && (
                  <Button onClick={() => setIsDialogOpen(true)}>
                    <Plus className="h-4 w-4 mr-2" />
                  ...
                  {filteredEvenements.map((evenement) => (
                    <TableRow key={evenement.id}>
                      <TableCell className="font-medium">
                        {evenement.nom || <span className="text-gray-400 italic">Sans nom</span>}
                      </TableCell>
                      <TableCell className="font-medium">
                      ...
                      <TableCell>
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            <div className="font-medium">{evenement.lycee.nom}</div>
                      ...
                      <TableCell>
                        {evenement.ville || <span className="text-gray-400 italic">Non spécifiée</span>}
                      </TableCell>
                      <TableCell>
                        <div className="space-y-1">
                          {evenement.evenementExposants.slice(0, 2).map((ee) => (
                            <Badge key={ee.id} variant="outline" className="text-xs mr-1">
                              {ee.exposant.domaine}
                            </Badge>
                          ))}
                      ...
                      <TableCell>
                        <Badge variant={evenement.lycee.type === "PUBLIC" ? "default" : "secondary"}>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>
              {editingEvenement ? "Modifier un événement" : "Ajouter un événement"}
            </DialogTitle>
            <DialogDescription>
              {editingEvenement 
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              <Select value={formData.lyceeId} onValueChange={(value) => 
            ...
            <div className="space-y-4">
              ...
              <div className="grid md:grid-cols-2 gap-3 max-h-60 overflow-y-auto border rounded-lg p-4">
                {exposants.map((exposant) => (
                  <div key={exposant.id} className="flex items-center space-x-2">
                    <Checkbox
                    ...
                    >
                      ...
                      <div className="text-xs text-gray-500">{exposant.domaine}</div>
                    </label>
                  </div>
                ))}
              </div>
              <p className="text-sm text-gray-500">
                {formData.exposantIds.length} exposant{formData.exposantIds.length > 1 ? 's' : ''} sélectionné{formData.exposantIds.length > 1 ? 's' : ''}
              </p>
            </div>
