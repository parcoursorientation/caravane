...
export default function AdminGaleriePage() {
      ...
      if (mediasResponse.ok) {
        ...
        const mockMedias: Media[] = [
          ...
          },
          {
            ...
            description:
              "Intervention d'un ingénieur de l'École Supérieure d'Ingénierie",
  ...
  useEffect(() => {
    ...
    if (categorieFilter !== "TOUS") {
      filtered = filtered.filter(
        (media) => media.categorie === categorieFilter
      );
    }
    ...
    if (evenementFilter !== "TOUS") {
      filtered = filtered.filter(
        (media) => media.evenement?.id === evenementFilter
      );
    }
  ...
  const handleCreateMedia = async () => {
    ...
    try {
      // Priorité: si une URL est fournie, on l'utilise telle quelle
      let fichierFinal = formData.fichierUrl?.trim();

      if (!fichierFinal) {
        // Sinon, si un fichier est sélectionné, on l'uploade d'abord
        if (formData.fichier) {
          ...
          fichierFinal = uploadResult.url;
        }
      }

      if (!fichierFinal) {
        throw new Error("Veuillez fournir une URL ou télécharger un fichier");
      }
      ...
      const response = await fetch("/api/admin/galerie", {
        ...
        body: JSON.stringify({
          ...
          evenementId:
            formData.evenementId === "none"
              ? null
              : formData.evenementId || null,
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.error || "Erreur lors de la création du média"
        );
      }
  ...
  const handleUpdateMedia = async () => {
    ...
    try {
      // Priorité à l'URL saisie si fournie; sinon conserver existant; sinon uploader un nouveau fichier
      let fichierFinal = formData.fichierUrl?.trim() || selectedMedia.fichier;
  ...
  const handleDeleteMedia = async (id: string) => {
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(
          errorData.error || "Erreur lors de la suppression du média"
        );
      }
        ...
        <Card className="mb-6">
          <CardContent className="p-6">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
              ...
              <Select
                value={evenementFilter}
                onValueChange={setEvenementFilter}
              >
                <SelectTrigger>
                ...
                <SelectContent>
                  ...
                  {evenements.map((evenement) => (
                    <SelectItem key={evenement.id} value={evenement.id}>
                      {evenement.nom || evenement.lycee.nom} -{" "}
                ...
                <CardContent className="p-4">
                  ...
                  {media.evenement && (
                    <div className="flex items-center gap-1 text-xs text-gray-400 mb-2">
                      ...
                      <span>
                        {media.evenement.nom || media.evenement.lycee.nom} -{" "}
                        {formatDate(media.evenement.date)}
                      </span>
                    </div>
                  )}
                    ...
                    <AlertDialog>
                      ...
                      <AlertDialogContent>
                        <AlertDialogHeader>
                          ...
                          <AlertDialogDescription>
                            Êtes-vous sûr de vouloir supprimer "{media.titre}" ?
    ...
    <div className="space-y-4">
      ...
      <div>
        ...
        <Select
          ...
          onValueChange={(value) =>
            setFormData({ ...formData, evenementId: value })
          }
          disabled={saving}
        >
          <SelectTrigger>
      ...
      <div>
        <Label htmlFor="fichier">Fichier ou URL *</Label>
        {/* Saisie URL directe */}
        <Input
          id="fichierUrl"
          placeholder="https://exemple.com/image.jpg ou /uploads/xxx.jpg"
          value={formData.fichierUrl}
          onChange={(e) =>
            setFormData({ ...formData, fichierUrl: e.target.value })
          }
          disabled={saving}
          className="mb-3"
        />

        {/* Upload fichier local (optionnel si URL fournie) */}
        <div
        ...
        {!(formData.fichierUrl?.trim() || formData.fichier) && (
          <p className="text-xs text-red-500 mt-1">
            Fournissez une URL ou téléversez un fichier
          </p>
        )}
      </div>
